---
title: "grafana"
description: "Grafana es una herramienta de código abierto para la visualización y analítica de series de tiempo, diseñada para consultar, visualizar, alertar y comprender tus métricas dondequiera que se almacenen."
...
*   **Fuente de Datos (Data Source)**: Una conexión a un sistema de base de datos o [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") donde se almacenan tus métricas o logs (ej. [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado."), [Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene."), InfluxDB, MySQL, PostgreSQL, Loki, [Jaeger](/web/backend/framework/herramientas/jaeger "Jaeger es un sistema de trazabilidad distribuida de código abierto, inspirado en Dapper de Google y OpenZipkin."), CloudWatch).
...
*   **Query (Consulta)**: El lenguaje específico del data source que se utiliza para recuperar datos para un panel (ej. PromQL para [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado."), KQL para [Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene.")).
...
*   **`[auth]`**: Integración con OAuth ([GitHub](/herramientas/github "GitHub es una plataforma web para el alojamiento de repositorios de control de versiones Git."), Google), LDAP.
...
3.  Configura los detalles de conexión (ej. URL de [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado."): `http://localhost:9090`).
...
*   **Query (Pestaña `Query`)**:
    ...
    2.  **Escribe tu consulta**:
        *   **[Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.") (PromQL)**: `rate(http_requests_total[5m])`
        *   **[Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene.") (KQL o Lucene)**: `http.method:POST AND http.status_code:[400 TO 599]`
        *   **[SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") (ej. PostgreSQL)**: `SELECT time_column, value_column FROM my_table WHERE $__timeFilter(time_column)` ( `$__timeFilter` es una macro de Grafana)
...
1.  **Configurar Variable**:
    ...
    *   **`Type`**:
        *   `Query`: Obtener valores de una consulta a la fuente de datos (ej. `label_values(up, job)` para jobs de [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.")).
...
*   **Utiliza KQL/PromQL Directamente**: Si tus datos están en [Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene.") o [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado."), aprende sus respectivos lenguajes de consulta. Grafana los integra perfectamente.
...
*   **Optimización del Rendimiento**:
    ...
    *   Optimiza tus consultas en el backend ([Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene."), [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.")) para que sean lo más eficientes posible.
