---
title: "husky"
description: "Husky es una utilidad que simplifica el uso de Git diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.\") Hooks."
...
**Husky** es una utilidad que simplifica el uso de **[Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") Hooks**. Los Git Hooks son scripts personalizados que Git ejecuta automáticamente en puntos específicos de su ciclo de vida (antes de un commit, después de un push, etc.). Husky hace que sea fácil gestionar y compartir estos hooks en un proyecto de equipo, almacenándolos en `package.json` (o `.husky/` en versiones recientes) en lugar de en el directorio `.git/`.

---

## 1. 🌟 Conceptos Clave

* **[Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") Hooks**: Son scripts de shell que Git ejecuta automáticamente antes o después de eventos específicos como `commit`, `push`, `merge`, etc.
...
* **Husky**: Una herramienta que gestiona los [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") Hooks. Instala los hooks en el directorio `.git/hooks/` de forma que apuntan a scripts controlados por `npm` (o `yarn`, `pnpm`), lo que permite que se versionen y compartan fácilmente entre desarrolladores.
...
1. **Instalar Husky:**
   ...
   pnpm add -D husky
   ```
2. **Habilitar [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") Hooks (Post-instalación):**

   * Este comando crea el directorio `.husky/` y configura [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") para que lo use.
...
* **`npx husky uninstall`**: Desinstala Husky y limpia los hooks de [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.").
...
* **`exit 1` en caso de Fallo**: Si un hook falla (ej. linter encuentra errores, pruebas no pasan), el script debe salir con un código de estado distinto de cero (`exit 1`) para que [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") aborte la operación.
