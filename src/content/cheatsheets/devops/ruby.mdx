...
description: "Ruby es un lenguaje orientado a objetos con una filosofía centrada en la felicidad del programador, diseñado por Yukihiro Matsumoto (\"Matz\") en 1995."
...
* **`nil` vs `false`**: Solo `nil` y `false` son falsy en Ruby. `0`, `""`, `[]` son **truthy** (diferente de [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.")/JS).
...
* **`0`, `""`, `[]` son truthy en Ruby**: A diferencia de [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.") y JavaScript, en Ruby solo `nil` y `false` son falsy. `if 0` ejecuta el bloque. Esto sorprende constantemente a programadores de otros lenguajes.
...
* **Usa `Pathname` en lugar de `File` y `Dir`**: La [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") orientada a objetos de Pathname es más legible y componible.

* **`pp` para debugging de estructuras**: `pp objeto` es como `p` pero con pretty-printing de hashes y arrays anidados.

* **Rake para orquestar tareas complejas**: Si tu script [bash](/devops/bash "Bash (Bourne Again Shell) es un shell de Unix y lenguaje de comandos compatible con POSIX.") crece más allá de 50 líneas de lógica de control, migra a un Rakefile con tareas bien definidas y documentadas.
