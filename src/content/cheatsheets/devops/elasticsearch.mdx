---
title: "elasticsearch"
description: "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene."
...
*   **Índice (Index)**: Una colección lógica de documentos que comparten un propósito similar (similar a una "tabla" en [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros."), pero más flexible).
...
Todas las interacciones con Elasticsearch se realizan a través de una [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") RESTful (generalmente JSON sobre HTTP). Puedes usar `curl`, [Postman](/herramientas/postman "Postman es una plataforma colaborativa para el desarrollo de APIs."), [Kibana](/herramientas/visualizacion_datos/kibana "Kibana es una interfaz de usuario de código abierto basada en web que permite visualizar y explorar los datos indexados en Elasticsearch.") Dev Tools o clientes de lenguaje.
...
*   **[Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.")**: `Elasticsearch Java Client` (cliente moderno), `Java High Level REST Client` (cliente anterior, ahora en modo mantenimiento).
*   **[Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.")**: `elasticsearch-py`.
*   **JavaScript**: `@elastic/elasticsearch`.
*   **[Go](/go/go "Go es un lenguaje compilado, concurrente y de tipado estático creado por Google.")**: `go-elasticsearch`.
...
*   **Monitoreo**: Monitorea el clúster de Elasticsearch (uso de CPU, memoria, disco, salud de shards, JVM). [Kibana](/herramientas/visualizacion_datos/kibana "Kibana es una interfaz de usuario de código abierto basada en web que permite visualizar y explorar los datos indexados en Elasticsearch.") Monitoring, Stack Monitoring son herramientas útiles.
...
Este cheatsheet te proporciona una referencia completa de Elasticsearch, cubriendo sus conceptos esenciales, arquitectura, cómo interactuar con él usando la [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") REST, la búsqueda con Query DSL, las agregaciones, los mappings y las mejores prácticas para construir sistemas de búsqueda y análisis potentes.