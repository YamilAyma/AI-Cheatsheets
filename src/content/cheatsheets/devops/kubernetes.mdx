...
2. **Configurar Acceso al Clúster**: `kubectl` usa un archivo de configuración (`~/.kube/config`) para conectarse a los clústeres.
   * Para clústeres locales (ej. Minikube, [Docker](/devops/docker "Docker es una plataforma de código abierto que utiliza la contenerización para empaquetar una aplicación y todas sus dependencias en una unidad estándar para el desarrollo de software.") Desktop Kubernetes): Generalmente se configura automáticamente.
...
Expone Servicios HTTP/HTTPS a clientes externos, proporcionando enrutamiento basado en host o path. Requiere un Ingress Controller (ej. [Nginx](/devops/nginx "Nginx (pronunciado 'engine-x') es un servidor web de alto rendimiento, servidor de correo reverso y balanceador de carga HTTP, diseñado para máxima concurrencia, eficiencia de recursos y bajo consumo de memoria.") Ingress Controller, Traefik).
...
Para usar en un Pod/Deployment:
    ...
    spec:
      ...
      containers:
        - name: my-container
          image: busybox
          [command](/arquitectura/design_patterns/comportamiento/command "El patrón Command es un patrón de comportamiento que encapsula una petición o acción como un objeto independiente, desacoplando el emisor de la petición del receptor que la ejecuta."): ["sh", "-c", "echo 'Hello from volume' > /data/hello.txt && cat /data/hello.txt"]
...
metadata:
  ...
  name: pod-reader
rules:
  - apiGroups: [""] # "" indica el core [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") group
