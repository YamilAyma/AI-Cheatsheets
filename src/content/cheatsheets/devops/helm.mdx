---
title: "helm"
description: "Helm es el gestor de paquetes para Kubernetes es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.\")."
...
**Helm** es el gestor de paquetes para [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores."). Te ayuda a definir, instalar y actualizar incluso las aplicaciones de Kubernetes más complejas. Los "paquetes" de Helm se llaman **Charts**.

---

## 1. 🌟 Conceptos Clave

*   **Chart (Gráfico)**: Un paquete de Helm. Contiene toda la información necesaria para definir una aplicación [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores."), incluyendo configuraciones y manifiestos (YAMLs) de Kubernetes, y puede ser versionado. Es la "unidad" desplegable en Helm.
*   **Release (Lanzamiento)**: Una instancia de un Chart desplegada en un clúster de [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores."). Cuando instalas un Chart, Helm crea una Release. Puedes tener múltiples Releases del mismo Chart.
...
*   **Templates (Plantillas)**: Los archivos YAML de manifiesto de [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.") dentro de un Chart que contienen lógica de plantilla ([Go](/go/go "Go es un lenguaje compilado, concurrente y de tipado estático creado por Google.") templating) para rellenar valores dinámicamente.
...
2.  **Verificar Instalación:**
    ...
    helm version
    ```
3.  **Configurar Acceso al Clúster [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.")**: Helm utiliza la configuración de `kubectl` (`~/.kube/config`). Asegúrate de que `kubectl` pueda conectarse a tu clúster.
...
*   Genera la estructura de un Chart vacío.
    ...
    *   Esto crea un directorio `my-new-app/` con una estructura estándar:
        ...
        *   `templates/`: Contiene los manifiestos de [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.") (YAMLs) con templating.
...
*   **`helm get manifest <release-name>`**: Muestra los manifiestos [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.") renderizados de una Release.
...
Helm utiliza [Go](/go/go "Go es un lenguaje compilado, concurrente y de tipado estático creado por Google.") templating para procesar los manifiestos de Kubernetes.
...
*   **Variables de Ámbito (`.Capabilities`)**: Información sobre el clúster (versión de K8s, [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.")).
