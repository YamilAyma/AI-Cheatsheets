...
* **Isla de Interactividad**: Un componente de framework de UI ([React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario."), Vue, Svelte, etc.) que se carga y ejecuta en el cliente. El resto de la página es HTML estático.
...
Permite construir endpoints [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") RESTful o GraphQL directamente en tu proyecto Astro.
