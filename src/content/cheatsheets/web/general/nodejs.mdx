---
title: "nodejs"
description: "Node.js es un entorno de ejecución de JavaScript basado en el motor V8 de Chrome."
...
* **Seguridad**: Sé consciente de las vulnerabilidades comunes (XSS, CSRF, [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") Injection) y usa librerías de seguridad (ej. Helmet para Express).
* **Pruebas**: Escribe pruebas unitarias e de integración para tu código (ej. [Jest](/web/frontend/testing/jest "Jest es un framework de testing para JavaScript y TypeScript mantenido por Meta, diseñado para ofrecer un ciclo de desarrollo rápido, configuración mínima y herramientas integradas para aserciones, mocking, cobertura y snapshots."), Mocha).
* **Nodemon**: Usa `nodemon` en desarrollo para recargar automáticamente tu servidor al detectar cambios en los archivos.
* **Frameworks Web**: Para la mayoría de las aplicaciones web, usa un framework como Express, [NestJS](/web/backend/framework/nestjs/nestjs "NestJS es un framework progresivo para Node.js diseñado para construir aplicaciones backend escalables, mantenibles y altamente testeables.") o Hapi para acelerar el desarrollo.
