...
description: "Spring Cloud Config proporciona un servidor y clientes para una configuración externa y distribuida en arquitecturas de microservicios."
...
*   **Centralización**: Todas las configuraciones de los microservicios se almacenan en un único lugar (ej. un repositorio [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.")).
*   **Versionado**: La configuración se gestiona con un sistema de control de versiones ([Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") por defecto), lo que permite rastrear cambios, revertir versiones y auditar.
...
3.  **Configurar `application.yml` (para el Config Server):**
    ...
    ```
    *   **Repositorio de Configuración (Ejemplo [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia."))**: Crea un repositorio Git (ej. en [GitHub](/herramientas/github "GitHub es una plataforma web para el alojamiento de repositorios de control de versiones Git.")) que contendrá tus archivos de configuración.
        ```
...
El Config Server expone un [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") REST para acceder a las configuraciones.
...
*   **Endpoint Básico**: `http://localhost:8888/{application}/{profile}/{label}`
    ...
    *   `{label}`: Etiqueta/rama de [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") (ej. `main`, `develop`, `v1.0`).
*   **Ejemplos**:
...
3.  **Anotar componentes con `@RefreshScope`:**
    *   Cualquier bean de Spring que necesite refrescar sus propiedades cuando cambie la configuración debe estar anotado con `@RefreshScope`.
    ...
    ```
4.  **Disparar el Refresh (después de cambiar la config en [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") y hacer push):**
    ```bash
...
1.  **Añadir dependencia en `pom.xml` (en el Config Server Y en los Clientes):**
    ...
    ```
3.  **Disparar el Refresh (después de cambiar la config en [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.") y hacer push):**
...
*   **Config Server**:
    *   Asegura el Config Server con [Spring Security](/web/backend/framework/spring/spring-security "Spring Security es un pilar fundamental para la seguridad en aplicaciones Spring y Spring Boot."). Solo los clientes autorizados deben poder acceder a la configuración.
...
*   **Repositorio [Git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.")**:
