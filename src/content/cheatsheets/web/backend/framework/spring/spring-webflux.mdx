...
2.  **Configuración de la Base de Datos (ej. R2DBC en `application.properties`):**
    ...
    ```
    *   **¡Importante!**: Si incluyes `spring-boot-starter-web` (Spring MVC) y `spring-boot-starter-webflux` en el mismo proyecto, [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") elegirá WebFlux si estás en Reactive Stack, o MVC si estás en Servlet Stack. Lo mejor es usar **solo uno** de los starters web.
...
*   Spring Data proporciona repositorios reactivos para bases de datos no bloqueantes (R2DBC para [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros."), Reactive [MongoDB](/base_datos/nosql/mongodb "MongoDB es una base de datos NoSQL orientada a documentos que almacena datos en formato BSON (Binary JSON), permitiendo esquemas flexibles y escalabilidad horizontal nativa."), Reactive [Redis](/base_datos/nosql/redis "Redis es un almacén de datos en memoria, de código abierto y alto rendimiento, que actúa como base de datos, caché, broker de mensajes y motor de streaming."), etc.).
