...
description: "Spring Authorization Server (SAS) es un proyecto de Spring Security que implementa la especificación OAuth 2.1 y OpenID Connect 1.0."
...
**Spring Authorization Server (SAS)** es un proyecto de [Spring Security](/web/backend/framework/spring/spring-security "¡Excelente elección! Spring Security es un pilar fundamental para la seguridad en aplicaciones Spring y Spring Boot.") que implementa la especificación OAuth 2.1 y OpenID Connect 1.0. Permite a las aplicaciones actuar como un **Servidor de Autorización**, emitiendo tokens de acceso a clientes autenticados después de obtener la autorización del propietario del recurso.
...
2.  **Esquemas de Base de Datos**: Spring Authorization Server proporciona scripts [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") para crear las tablas necesarias (`oauth2_registered_client`, `oauth2_authorization`, `oauth2_authorization_consent`). Busca `oauth2-authorization-server-schema.sql` en el JAR de la librería.
...
Para que otra aplicación [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") actúe como Cliente de OAuth2 e inicie sesión vía SAS.
