...
description: "El Spring AOT Engine (Ahead-of-Time Engine) es un conjunto de herramientas y un proceso dentro de Spring Boot 3+ que permite transformar una aplicación Spring Boot estándar en un ejecutable nativo compilado."
...
El **Spring AOT Engine** (Ahead-of-Time Engine) es un conjunto de herramientas y un proceso dentro de **[Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") 3+** que permite transformar una aplicación Spring Boot estándar en un ejecutable nativo compilado. Esto se logra mediante la generación de metadatos de configuración para **[GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.") Native Image** en tiempo de compilación (AOT), lo que permite a las aplicaciones Spring beneficiarse de arranques instantáneos y una huella de memoria reducida.
...
*   **[GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.") Native Image**: La tecnología de GraalVM que compila aplicaciones [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") a ejecutables binarios independientes y ligeros.
*   **Dinamismo en [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.")**: Características como la reflexión, la carga dinámica de clases y los proxies son muy usadas en frameworks como Spring. [GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.") Native Image, al compilar AOT, no puede ver estas llamadas dinámicas.
*   **Metadata (Runtime Hints)**: Spring AOT Engine analiza el código de tu aplicación Spring y genera los metadatos necesarios (hints) para [GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.") Native Image, instruyéndole sobre qué elementos dinámicos necesita incluir en el ejecutable nativo.
*   **Procesamiento AOT**: La fase del proceso de construcción donde [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") analiza la aplicación, sus dependencias y las librerías de Spring para generar el código y los metadatos AOT.
...
*   **[Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") 3.0+**: Es esencial. Las versiones anteriores no tienen soporte AOT.
*   **[GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.")**: Necesitas una instalación de GraalVM (Community Edition o Enterprise) y el componente `native-image` instalado (`gu install native-image`).
...
2.  **Procesamiento AOT (Phase 1: `process-aot`)**:
    ...
    *   Genera código fuente [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") adicional que contiene los `RuntimeHints` (metadatos sobre reflexión, proxies, etc.) y un contexto de Spring optimizado para AOT.
    *   Este código generado se guarda en `target/spring-aot/main/sources` (o similar).
3.  **Compilación del Código Generado**: El compilador [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") compila este código generado a `.class`.
...
5.  **Compilación a Native Image (`compile-native`)**:
    *   El `native-maven-plugin` invoca a la herramienta `native-image` de [GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java.").
...
*   **Tipos de Hints**:
    ...
    *   **[Proxy](/arquitectura/design_patterns/estructurales/proxy "El patrón Proxy es un patrón estructural que proporciona un sustituto o intermediario para controlar el acceso a un objeto real, añadiendo comportamiento transversal sin modificar su implementación original.")**: Qué interfaces se usan para proxies dinámicos.
...
*   **Generación Automática**: Spring AOT Engine, junto con las librerías de [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") y Spring Framework, ya tienen muchos hints predefinidos para la mayoría de los casos de uso comunes.
...
*   **Arranque Instantáneo**: Las aplicaciones [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") que tardaban segundos en arrancar, ahora lo hacen en milisegundos. Ideal para serverless, funciones como servicio (FaaS) y microservicios efímeros.
...
*   **Compromisos de Rendimiento de Pico**: Para aplicaciones de [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") que tienen una ejecución *muy* larga (horas o días), el compilador JIT de una JVM tradicional (con Warm-up) podría eventualmente superar el rendimiento de pico de un ejecutable nativo.
...
*   **Empieza con [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") 3+**: Asegúrate de que tu proyecto esté en la versión adecuada.
...
*   **Spring Native (Legado)**: Si estás en un proyecto con [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") 2.x, el soporte nativo se llamaba "Spring Native". Spring AOT Engine es la evolución y parte del core de Spring Boot 3+.

---

Este cheatsheet te proporciona una referencia completa de Spring AOT Engine, cubriendo sus conceptos esenciales, el proceso de construcción para ejecutables nativos con [GraalVM](/herramientas/machines/graalvm "GraalVM es una máquina virtual universal de alto rendimiento que extiende la JVM de Java."), la gestión de `RuntimeHints` y las mejores prácticas para construir aplicaciones Spring Boot de alto rendimiento y bajo consumo de recursos.