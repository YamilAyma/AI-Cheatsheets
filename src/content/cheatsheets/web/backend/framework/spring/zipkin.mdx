---
title: "zipkin"
description: "Zipkin es un sistema de trazabilidad distribuida de código abierto."
...
*   **Binary Annotation (Anotación Binaria / Etiqueta)**: Pares clave-valor que proporcionan metadatos adicionales sobre un Span (ej. `http.method`, `http.url`, `error`, `db.statement`). Son el equivalente a los "tags" en OpenTelemetry/[Micrometer Tracing](/web/backend/framework/spring/micrometer-tracing "Micrometer Tracing (parte de Spring Boot 3+ y Micrometer 1.10+) es un puente entre las aplicaciones Spring y las APIs de instrumentación de trazabilidad distribuida (principalmente OpenTelemetry).").
*   **Colector (Collector)**: El componente de Zipkin que recibe los Spans de los servicios instrumentados.
*   **Almacenamiento (Storage)**: Zipkin almacena las trazas en un backend (Memoria, MySQL, Cassandra, [Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene.")).
...
*   **MySQL**:
    ...
    docker run -d -p 9411:9411 \
      ...
      openzipkin/zipkin
    ```
*   **[Elasticsearch](/devops/elasticsearch "Elasticsearch es un motor de búsqueda y análisis distribuido en tiempo real, construido sobre Apache Lucene.")**:
    ```bash
    docker run -d -p 9411:9411 \
...
[Micrometer Tracing](/web/backend/framework/spring/micrometer-tracing "Micrometer Tracing (parte de Spring Boot 3+ y Micrometer 1.10+) es un puente entre las aplicaciones Spring y las APIs de instrumentación de trazabilidad distribuida (principalmente OpenTelemetry).") (el sucesor de Spring Cloud Sleuth) es la forma recomendada de instrumentar aplicaciones [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") para Zipkin (basado en OpenTelemetry).
...
*   **[Spring Cloud Gateway](/web/backend/framework/spring/spring-cloud-gateway "Spring Cloud Gateway es un framework API Gateway construido sobre Spring Framework 5, Spring Boot 2 y Project Reactor.")**: Si tu [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") Gateway (con Spring Cloud Gateway) también está instrumentado, las trazas se iniciarán y propagarán desde allí.
*   **Spring Cloud Sleuth (Legacy)**: Las versiones de [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") anteriores a 3.x usan Spring Cloud Sleuth, que se integra directamente con Brave (una librería de trazabilidad) para Zipkin. [Micrometer Tracing](/web/backend/framework/spring/micrometer-tracing "Micrometer Tracing (parte de Spring Boot 3+ y Micrometer 1.10+) es un puente entre las aplicaciones Spring y las APIs de instrumentación de trazabilidad distribuida (principalmente OpenTelemetry).") es la evolución compatible con OpenTelemetry.
...
*   **Logs**: Configura tus logs para incluir el `Trace ID` y `Span ID` para que puedas correlacionar fácilmente los logs con las trazas en Zipkin.
    *   **[Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.")**: Por defecto, los logs ya incluyen `[<app-name>,<trace-id>,<span-id>,<true/false>]`.
...
*   **Usa un [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") Gateway Instrumentado**: Si tienes un API Gateway (ej. [Spring Cloud Gateway](/web/backend/framework/spring/spring-cloud-gateway "Spring Cloud Gateway es un framework API es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.') Gateway construido sobre Spring Framework 5, [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") 2 y Project Reactor.")), asegúrate de que esté instrumentado. Será el punto de entrada de todas las trazas.
...
Este cheatsheet te proporciona una referencia completa de Zipkin, cubriendo sus conceptos esenciales de trazabilidad distribuida, cómo instalar el servidor, instrumentar aplicaciones [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.") con [Micrometer Tracing](/web/backend/framework/spring/micrometer-tracing "Micrometer Tracing (parte de Spring Boot 3+ y Micrometer 1.10+) es un puente entre las aplicaciones Spring y las APIs de instrumentación de trazabilidad distribuida (principalmente OpenTelemetry)."), visualizar trazas y aplicar las mejores prácticas para un monitoreo y depuración eficientes en arquitecturas de microservicios.