...
description: "Spring Data JPA es una parte del paraguas de Spring Data, que tiene como objetivo reducir la cantidad de código repetitivo necesario para implementar la capa de acceso a datos (Repository) en aplicaciones basadas en la tecnología JPA (Java Persistence API es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.\"))."
...
*   **`EntityManager`**: El [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") central de JPA para interactuar con el contexto de persistencia. Spring Data JPA lo gestiona por ti.
...
Las entidades son clases [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") que se mapean a tablas de base de datos.
...
*   **DTOs (Data Transfer Objects)**: No expongas tus entidades JPA directamente a la capa de controlador/vista. Mapea tus entidades a DTOs para la comunicación con el cliente. [ModelMapper](/java/librerias/modelmapper "ModelMapper es una librería de Java que simplifica el mapeo de objetos de un modelo a otro.") o [MapStruct](/java/librerias/mapstruct "MapStruct es un generador de código que simplifica la implementación de mapeos de datos entre clases de objetos de Java.") son útiles para esto.
...
*   **Consultas Nativas con Cuidado**: Usa `nativeQuery = true` solo cuando no puedas lograr la consulta deseada con [JPQL](/java/jpql "JPQL es el lenguaje de consulta orientado a objetos definido como parte de la especificación Java Persistence API (JPA).") o métodos derivados. Las consultas nativas son menos portátiles entre diferentes bases de datos.
