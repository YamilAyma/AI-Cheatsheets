...
* **`render_template()`**: Función para cargar y renderizar una plantilla.
  ...
  ```
* **Sintaxis [Jinja2](/template_engine/jinja2 "Jinja2 es un motor de plantillas (template engine) para Python.") (`templates/index.html`)**:
...
* **Despliegue**: No uses `app.run(debug=True)` en producción. Usa un servidor WSGI como Gunicorn o uWSGI, y un servidor web como Nginx o Apache como [proxy](/arquitectura/design_patterns/estructurales/proxy "El patrón Proxy es un patrón estructural que proporciona un sustituto o intermediario para controlar el acceso a un objeto real, añadiendo comportamiento transversal sin modificar su implementación original.") inverso.
