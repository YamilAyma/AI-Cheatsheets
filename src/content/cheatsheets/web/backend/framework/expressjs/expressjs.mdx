---
title: "expressjs"
description: "Express.js es un framework web rápido, sin opiniones (unopinionated) y minimalista para Node.js."
...
* **Variables de Entorno**: Utiliza variables de entorno (ej. con `dotenv` para desarrollo) para configurar la aplicación y almacenar secretos (ej. claves de [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí."), credenciales de base de datos).
...
* **Seguridad**: Usa librerías de seguridad (ej. `helmet` para protección HTTP, `cors` para CORS) y ten en cuenta las vulnerabilidades comunes (XSS, CSRF, inyección [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.")).
