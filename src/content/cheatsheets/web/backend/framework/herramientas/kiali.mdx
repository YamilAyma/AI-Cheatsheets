---
title: "kiali"
description: "Kiali es una consola de operaciones para Istio de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.\"), que proporciona un mapa de la malla de servicios, gráficos de flujo de tráfico, vistas detalladas de la salud de los servicios, y la capacidad de depurar y validar la configuración de Istio."
...
**Kiali** es una consola de operaciones para [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio."), que proporciona un mapa de la malla de servicios, gráficos de flujo de tráfico, vistas detalladas de la salud de los servicios, y la capacidad de depurar y validar la configuración de Istio. Permite a los desarrolladores y operadores visualizar el comportamiento de la malla, entender las relaciones entre servicios y detectar problemas de manera eficiente.
...
*   **[Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.") Configuration Validation**: Kiali puede validar tu configuración de Istio (VirtualServices, DestinationRules, Gateways, etc.) y señalar posibles errores o inconsistencias.
*   **Trace Integration (Integración de Trazas)**: Se integra con herramientas de trazabilidad como [Jaeger](/web/backend/framework/herramientas/jaeger "Jaeger es un sistema de trazabilidad distribuida de código abierto, inspirado en Dapper de Google y OpenZipkin.") para mostrar los detalles de las trazas de las solicitudes.
*   **Metrics Integration (Integración de Métricas)**: Utiliza [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.") (el backend de métricas de [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.")) para mostrar métricas detalladas de servicios y cargas de trabajo.

---

## 2. 🛠️ Instalación y Acceso

Kiali se instala como parte del ecosistema de [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.").

1.  **Asegúrate de que [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.") esté instalado y funcionando.**
2.  **Instalar Kiali (con `istioctl`)**:
    *   Una forma común es con el perfil de instalación de [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.") (ej. `demo` o `default` suelen incluir Kiali).
    *   Si no lo tienes, puedes instalarlo específicamente:
        ...
        ```
    *   También puedes instalarlo a través del repositorio de [Helm](/devops/helm "Helm es el gestor de paquetes para Kubernetes.") de [Istio](/web/backend/framework/herramientas/istio "Istio es una malla de servicios (Service Mesh) de código abierto que proporciona una capa de infraestructura transparente para gestionar las comunicaciones de servicio a servicio.").
3.  **Acceder a la UI de Kiali**:
    ```bash
    istioctl dashboard kiali
    ```
    *   Esto suele abrir un navegador en la URL correcta (ej. `http://localhost:20001`).
    *   Kiali te pedirá credenciales de [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.") (normalmente las de tu `kubectl` config).
...
*   Proporciona una visión [general](/arquitectura/general "La Arquitectura del Software es la estructura fundamental de un sistema de software y la disciplina para crear tales estructuras.") de la salud de tus namespaces y cargas de trabajo.
...
*   Lista todos los servicios de [Kubernetes](/devops/kubernetes "Kubernetes (K8s) es una plataforma portable y extensible de código abierto para gestionar cargas de trabajo y servicios en contenedores.").
...
*   **Integración con [Jaeger](/web/backend/framework/herramientas/jaeger "Jaeger es un sistema de trazabilidad distribuida de código abierto, inspirado en Dapper de Google y OpenZipkin.")**: Kiali se conecta a Jaeger (el backend de trazabilidad de Istio por defecto) y muestra las trazas de las solicitudes a través de tus servicios.
*   Puedes filtrar trazas por servicio, operación, duración, etc.
*   Haciendo clic en una traza, te redirige a la interfaz de [Jaeger](/web/backend/framework/herramientas/jaeger "Jaeger es un sistema de trazabilidad distribuida de código abierto, inspirado en Dapper de Google y OpenZipkin.") para ver los detalles completos del Span.
...
Kiali utiliza [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.") (el sistema de métricas de Istio) para mostrar gráficos de métricas detallados.
...
*   **Dashboards Personalizados**: Aunque Kiali proporciona gráficos predeterminados, también puedes configurar tus propios dashboards en [Grafana](/herramientas/visualizacion_datos/grafana "Grafana es una herramienta de código abierto para la visualización y analítica de series de tiempo, diseñada para consultar, visualizar, alertar y comprender tus métricas dondequiera que se almacenen.") usando las métricas de [Prometheus](/web/backend/framework/herramientas/prometheus "Prometheus es una solución de monitoreo de código abierto y sistema de alertado.") que Istio y Kiali exponen.
...
*   **Integra Tracing ([Jaeger](/web/backend/framework/herramientas/jaeger "Jaeger es un sistema de trazabilidad distribuida de código abierto, inspirado en Dapper de Google y OpenZipkin."))**: Asegúrate de que tu sistema de trazabilidad (Jaeger por defecto con Istio) esté configurado y enviando datos para que Kiali pueda mostrar las trazas.
