---
title: "zustand"
description: "¡Claro que sí! Zustand es una librería de gestión de estados para React (y vainilla JavaScript) conocida por su simplicidad, ligereza y rendimiento."
...
¡Claro que sí! Zustand es una librería de gestión de estados para [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") (y vainilla JavaScript) conocida por su simplicidad, ligereza y rendimiento. Es una alternativa popular para quienes buscan una solución menos "opinionated" y con menos boilerplate que [Redux](/web/frontend/general/react/redux "Redux es una librería de JavaScript para gestionar el estado de la aplicación.").
...
**Zustand** es una librería de gestión de estados pequeña y rápida para [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") y JavaScript/[TypeScript](/web/general/typescript "  TypeScript es una herramienta fundamental en el desarrollo web moderno, especialmente para proyectos a gran escala, ya que añade seguridad de tipo y mejoras en la productividad."). Se basa en una [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") sencilla con hooks y evita el boilerplate, las inyecciones de proveedores (`Provider`) y el contexto complejo, lo que la hace muy fácil de aprender y usar.
...
*   **Hooks-based**: Interactúa con el store dentro de componentes [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") usando hooks generados automáticamente.
*   **No Boilerplate**: Mínimo código para configurar y usar.
*   **No `Provider` Component**: No necesitas envolver tu aplicación con un componente `Provider` (como en [Redux](/web/frontend/general/react/redux "Redux es una librería de JavaScript para gestionar el estado de la aplicación.") o Context [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.")). Los stores se pueden usar directamente en cualquier componente.
...
*   **Direct Access**: Puedes acceder y manipular el estado fuera de los componentes de [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario."), lo que es útil para lógica de negocio en servicios o utilidades.
...
*   **`devtools`**: Para integrar con la extensión [Redux](/web/frontend/general/react/redux "Redux es una librería de JavaScript para gestionar el estado de la aplicación.") DevTools.
    ```javascript
    import { create } from 'zustand';
    ...
    ```
*   **`immer`**: Simplifica las actualizaciones de estado inmutables usando una sintaxis mutable (como en [Redux](/web/frontend/general/react/redux "Redux es una librería de JavaScript para gestionar el estado de la aplicación.") Toolkit).
    ```javascript
    import { create } from 'zustand';
...
*   **[TypeScript](/web/general/typescript "TypeScript es una herramienta fundamental en el desarrollo web moderno, especialmente para proyectos a gran escala, ya que añade seguridad de tipo y mejoras en la productividad.")**: Zustand tiene un excelente soporte de TypeScript. Define las interfaces para tu estado y para la función del store para una experiencia de desarrollo tipo-segura.
    ```typescript
...
Este cheatsheet te proporciona una referencia completa de Zustand, cubriendo sus conceptos esenciales, cómo crear y usar stores, manejar lógica asíncrona, optimizar re-renders, persistir el estado y aplicar middlewares, junto con las mejores prácticas para construir aplicaciones [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") eficientes y fáciles de mantener.