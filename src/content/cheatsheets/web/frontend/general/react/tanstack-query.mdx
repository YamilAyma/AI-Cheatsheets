...
description: "¡Excelente iniciativa! Un \"cheatsheet\" completo y bien estructurado de TanStack Query (anteriormente React Query) es una herramienta invaluable."
...
¡Excelente iniciativa! Un "cheatsheet" completo y bien estructurado de TanStack Query (anteriormente [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") Query) es una herramienta invaluable. He preparado uno que es conciso pero cubre los aspectos más importantes, con ejemplos claros y puntos clave.
...
* **`QueryClientProvider`**: El componente [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") que envuelve tu aplicación, proveyendo la instancia de `QueryClient` a todos tus componentes.
...
const {
  ...
  mutationFn: async (variables) =&gt; { // **FUNCIÓN ASÍNCRONA OBLIGATORIA**
    const res = await fetch('/[api](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.")/crear-item', {
      method: 'POST',
...
const addTodo = async (newTodo) =&gt; {
  const res = await fetch('/[api](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.")/todos', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
function InfinitePostList() {
  const {
    ...
    queryKey: ['posts', 'infinite'],
    queryFn: fetchPosts,
    getNextPageParam: (lastPage) =&gt; lastPage.nextCursor, // Asume que la [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") devuelve { posts: [], nextCursor: '...' }
    initialPageParam: 0,
  });
