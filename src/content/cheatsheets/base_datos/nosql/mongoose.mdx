...
* **Validación antes de persistir**: Mongoose valida tipos, requeridos, enums, regex y validadores custom antes de enviar a [MongoDB](/base_datos/nosql/mongodb "MongoDB es una base de datos NoSQL orientada a documentos que almacena datos en formato BSON (Binary JSON), permitiendo esquemas flexibles y escalabilidad horizontal nativa."). Falla rápido y local.
...
* **[TypeScript](/web/general/typescript "TypeScript es una herramienta fundamental en el desarrollo web moderno, especialmente para proyectos a gran escala, ya que añade seguridad de tipo y mejoras en la productividad.") genéricos sin `InferSchemaType`**: Duplicar interfaces y schema causa drift.
...
* **Versionado semántico en cambios de schema**: Cambios breaking (renombrar campos, cambiar tipos) requieren migración + nueva versión de [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.").
