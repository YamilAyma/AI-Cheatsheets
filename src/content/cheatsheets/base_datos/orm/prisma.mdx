...
* **Cliente autogenerado**: `prisma generate` produce `@prisma/client` con tipos [TypeScript](/web/general/typescript "TypeScript es una herramienta fundamental en el desarrollo web moderno, especialmente para proyectos a gran escala, ya que añade seguridad de tipo y mejoras en la productividad.") derivados del schema. Autocompletado, validación de tipos y detección temprana de errores.
...
* **Migraciones declarativas**: `prisma migrate dev` genera [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") de migración a partir del diff del schema. Versionadas en [git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia."), reproducibles, aplicables en CI.
* **Query [Builder](/arquitectura/design_patterns/creacionales/builder "El patrón Builder es un patrón creacional que separa la construcción de un objeto complejo de su representación final, permitiendo que el mismo proceso de ensamblaje genere diferentes variantes.") fluido**: [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") encadenable y composable (`prisma.user.findMany({ where: {...}, include: {...} })`). No es [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") stringificado, es código tipado.
...
* **`skip`/`take` para offset pagination**:
  ...
  ```
* **Connection pooling**: Prisma usa el pool del driver. En serverless, usa [singleton](/arquitectura/design_patterns/creacionales/singleton "El patrón Singleton es un patrón creacional estructural que garantiza que una clase tenga exactamente una instancia en todo el ciclo de vida de un ámbito definido, y proporciona un punto de acceso global a dicha instancia.") + Prisma Accelerate.
...
* **Crear múltiples instancias de `PrismaClient`**: En dev con hot-reload o en serverless, cada instancia agota conexiones.
  * Fix: [Singleton](/arquitectura/design_patterns/creacionales/singleton "El patrón Singleton es un patrón creacional estructural que garantiza que una clase tenga exactamente una instancia en todo el ciclo de vida de un ámbito definido, y proporciona un punto de acceso global a dicha instancia.") con `globalThis` (ver sección 2).
* **Olvidar `npx prisma generate` tras cambiar schema**: El cliente se queda con tipos viejos y [TypeScript](/web/general/typescript "TypeScript es una herramienta fundamental en el desarrollo web moderno, especialmente para proyectos a gran escala, ya que añade seguridad de tipo y mejoras en la productividad.") no detecta errores.
...
* **Serverless cold starts con Prisma**: Primera query tarda 200-500ms.
  * Fix: Usa Prisma Accelerate o Data [Proxy](/arquitectura/design_patterns/estructurales/proxy "El patrón Proxy es un patrón estructural que proporciona un sustituto o intermediario para controlar el acceso a un objeto real, añadiendo comportamiento transversal sin modificar su implementación original.") para connection pooling global.
...
* **Valida input con [Zod](/web/frontend/general/zod "¡Absolutamente! Zod es una librería increíble para la validación de esquemas y tipos en TypeScript/JavaScript, famosa por su simplicidad y excelente inferencia de tipos.") antes de Prisma**: `const data = schema.parse(body); await prisma.user.create({ data })`. Doble validación: schema en DB, Zod en entrada.
...
* **Versiona `migrations/` en [git](/herramientas/git/git "Git es un sistema de control de versiones distribuido (DVCS) diseñado para manejar proyectos pequeños o muy grandes con velocidad y eficiencia.")**: Es el histórico oficial de cambios de schema. Nunca ignores.
