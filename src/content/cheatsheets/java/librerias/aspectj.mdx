---
title: "aspectj"
description: "AspectJ es una extensión \"seamless\" (sin costuras) de Java que implementa la Programación Orientada a Aspectos (AOP)."
...
*   **Aspect (Aspecto)**: Una unidad modular que encapsula una preocupación transversal.
    *   **Notación**: En AspectJ, los aspectos se escriben usando un sintaxis extendida de [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") o anotaciones (estilo Spring AOP).
...
*   **Weaving (Tejido)**: El proceso de inyectar el código del aspecto (Advice) en los Join Points de la aplicación. Puede ocurrir en diferentes momentos:
    ...
    *   **Post-Compile Weaving (Binary/Load-Time Weaving - LTW)**: Después de compilar el código [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") (en bytecode). Para LTW, se necesita un agente Java en tiempo de ejecución.
...
Un aspecto es una clase [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.") especial con la extensión `.aj` o una clase Java con anotaciones específicas (estilo Spring AOP, pero AspectJ también las soporta).
...
Si estás trabajando en un entorno [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente."), a menudo usarás Spring AOP, que se basa en proxies y solo soporta `execution` Join Points en métodos públicos. Sin embargo, AspectJ puede interpretar estas anotaciones y hacer Compile-Time Weaving para un AOP más completo.
...
*   **`this()`**: Selecciona Join Points donde el objeto [proxy](/arquitectura/design_patterns/estructurales/proxy "El patrón Proxy es un patrón estructural que proporciona un sustituto o intermediario para controlar el acceso a un objeto real, añadiendo comportamiento transversal sin modificar su implementación original.") (si AOP usa proxies) es una instancia de un tipo dado.
...
2.  **`META-INF/aop.xml`**: Archivo de configuración que especifica qué aspectos tejer.
    ...
    ```
3.  **Configurar Agente [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.")**: Ejecutar la JVM con el agente `aspectjweaver`.
    ```bash
    java -javaagent:/path/to/aspectjweaver-1.9.21.jar -jar your_app.jar
    ```
    *   Para [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente."), puedes añadir la dependencia `spring-aspects` y usar `--spring.profiles.active=aspectj` o configurar explícitamente el agente.
...
Este cheatsheet te proporciona una referencia completa de AspectJ, cubriendo sus conceptos esenciales de AOP, cómo configurar el weaving, definir aspectos y consejos, usar expresiones de pointcut y aplicar las mejores prácticas para modularizar tus preocupaciones transversales de manera efectiva en aplicaciones [Java](/java/java "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos.").