---
title: "java"
description: "Java es un lenguaje de programación de propósito general, concurrente, basado en clases y orientado a objetos."
...
Java es un lenguaje de programación de propósito [general](/arquitectura/general "La Arquitectura del Software es la estructura fundamental de un sistema de software y la disciplina para crear tales estructuras."), concurrente, basado en clases y orientado a objetos. Es una plataforma con un ecosistema maduro y extenso, utilizado para construir aplicaciones empresariales, móviles (Android), de escritorio, web, servicios en la nube y Big Data.
...
* **Amplia Biblioteca Estándar ([API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí."))**: Una rica colección de clases y paquetes para tareas comunes (E/S, red, colecciones, etc.).
...
* **Sincronización**: `synchronized` palabra clave o `java.util.concurrent` (Lock, Semaphore, ExecutorService).
  ...
  // synchronized (this) { /* ... */ }
  ```
* **`ExecutorService` (Java Concurrency [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí."))**: La forma moderna y recomendada para gestionar hilos.
  ```java
...
* **Pruebas Unitarias**: Utiliza frameworks como [JUnit](/java/junit "JUnit es un framework de código abierto para escribir y ejecutar pruebas unitarias en Java.") para escribir pruebas unitarias.
