---
title: "jinja2"
description: "Jinja2 es un motor de plantillas (template engine) para Python."
...
Jinja2 es un motor de plantillas (template engine) para [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos."). Su objetivo es generar documentos de texto (como HTML, XML, CSS, o cualquier formato de texto) a partir de un template y un conjunto de datos. Es rápido, seguro (con sandboxing) y fácil de extender.
...
1. **Instalación:**
   ...
   pip install Jinja2
   ```
2. **Uso Básico en [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos."):**
   ...
   * Crea una carpeta `templates/` en el mismo directorio que tu script [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.").
...
* **Formato [general](/arquitectura/general "La Arquitectura del Software es la estructura fundamental de un sistema de software y la disciplina para crear tales estructuras.")**: `{{ valor | filtro(argumentos) }}`
* **Comunes**:
  ...
  * `tojson(indent=None)`: Convierte un objeto [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.") a JSON.
  * `format(spec)`: Para formatear números/cadenas (similar al método `.format()` de [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.")).
    ```html
...
* **Formato [general](/arquitectura/general "La Arquitectura del Software es la estructura fundamental de un sistema de software y la disciplina para crear tales estructuras.")**: `{% if valor is test %}`
* **Comunes**:
...
* **Auto-escaping**: Jinja2 escapa automáticamente el HTML en las expresiones (`{{ ... }}`) para prevenir ataques XSS.
  ...
  * Si necesitas renderizar HTML sin escapar (ej. contenido rico guardado de forma segura), usa el filtro `|safe`. **¡Solo haz esto con contenido de confianza!**
  ...
  <p>{{ user_input | safe }}</p>
  ```
* **Sandboxing**: Prohíbe el acceso a atributos/métodos peligrosos en objetos [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.") pasados a las plantillas.

---

## 10. 💡 Buenas Prácticas y Consejos

* **Separación de Preocupaciones**: Mantén la lógica de la aplicación en [Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.") y la lógica de presentación en Jinja2. Evita cálculos complejos o manipulaciones de datos en las plantillas.
...
* **`url_for` (en [Flask](/web/backend/framework/flask/flask "Flask es un microframework web para Python que proporciona las herramientas esenciales para construir aplicaciones web, dejando la elección de otras herramientas (como ORM o templating) en manos del desarrollador.")/[Django](/web/backend/framework/django/django "Django es un framework web de alto nivel en Python que fomenta el desarrollo rápido y un diseño limpio y pragmático."))**: En frameworks web, usa las funciones proporcionadas por el framework (como `url_for` en Flask) para generar URLs, en lugar de escribirlas directamente.
