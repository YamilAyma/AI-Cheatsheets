---
title: "iterator"
description: "El patrón Iterator es un patrón de comportamiento que proporciona una forma estandarizada de recorrer secuencialmente los elementos de una estructura agregada sin exponer su representación interna."
...
```
Regla inquebrantable: El iterador **nunca** debe exponer índices internos, punteros crudos o permitir mutación directa de la colección a través de su [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí."), a menos que esté explícitamente diseñado y documentado como mutante seguro.
...
* **[Visitor](/arquitectura/design_patterns/comportamiento/visitor "El patrón Visitor es un patrón de comportamiento que separa algoritmos de las estructuras de objetos sobre las que operan, permitiendo añadir nuevas operaciones sin modificar las clases de los elementos visitados.")**: Aplica operaciones a estructura estable. Enfocado en múltiples traversals sin modificar nodos.
* **[Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.")**: Intercambia algoritmos completos. Enfocado en variación de lógica, no en navegación.
* **[Composite](/arquitectura/design_patterns/estructurales/composite "El patrón Composite es un patrón estructural que permite componer objetos en estructuras de árbol para representar jerarquías parte-todo.")**: Modela jerarquías parte-todo. Enfocado en uniformidad estructural, no en traversal.
...
* **Confundir con [Visitor](/arquitectura/design_patterns/comportamiento/visitor "El patrón Visitor es un patrón de comportamiento que separa algoritmos de las estructuras de objetos sobre las que operan, permitiendo añadir nuevas operaciones sin modificar las clases de los elementos visitados.") o [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.")**: Usar Iterator para aplicar operaciones complejas o variar lógica de procesamiento.
  * Fix: Si aplica múltiples operaciones a estructura → [Visitor](/arquitectura/design_patterns/comportamiento/visitor "El patrón Visitor es un patrón de comportamiento que separa algoritmos de las estructuras de objetos sobre las que operan, permitiendo añadir nuevas operaciones sin modificar las clases de los elementos visitados."). Si varía algoritmo completo → [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza."). Si secuencia acceso uniforme → Iterator. No mezcle propósitos.
...
* **[State](/arquitectura/design_patterns/comportamiento/state "El patrón State es un patrón de comportamiento que permite a un objeto alterar su comportamiento cuando su estado interno cambia, haciendo que parezca que el objeto cambia de clase.") Leakage en Iteradores Concurrentes**: Múltiples hilos comparten cursor o buffer sin sincronización. Corrupción de datos.
...
* **Prefiera Protocolos Nativos del Lenguaje**: `Iterable`/`Iterator` (JS), `__iter__`/`__next__` ([Python](/python/python "Python es un lenguaje de programación de alto nivel, interpretado, interactivo y orientado a objetos.")), `java.util.Iterator`, [Rust](/rust/rust "Rust es un lenguaje de programación de sistemas enfocado en seguridad, concurrencia y rendimiento sin recolector de basura (GC).") `std::iter::Iterator`. Evite reinvención innecesaria.
