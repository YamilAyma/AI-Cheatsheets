...
* **Explosión de Subclases por Combinación**: Crear `CsvToJsonValidator`, `XmlToCsvValidator`, `JsonToXmlValidator` para cada variante.
  * Fix: Separe dimensiones independientes. Use composición de pasos ([Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.") por paso) en lugar de herencia cruzada. Evite combinatoria en clases.
