...
* **Cliente**: Componente que parsea la entrada, construye el AST, inyecta el contexto y dispara `interpret()`. Desconoce la lógica interna de cada nodo.
  * Por qué importa: Mantiene inversión de dependencias. La capa de presentación o API solo interactúa con el motor de interpretación.
...
* **Confundir con Visitor o Command**: Usar Interpreter para añadir operaciones sin evaluación, o para encolar peticiones.
  * Fix: Si separa operaciones de estructura → Visitor. Si encapsula petición ejecutable → Command. Si evalúa gramática representada → Interpreter. No mezcle propósitos.
