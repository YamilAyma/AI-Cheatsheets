...
* **Doble Despacho (Double Dispatch)**: Mecanismo que resuelve el tipo de objeto en runtime dos veces: primero por el elemento (`accept`), luego por el visitante (`visit`).
  * Por qué importa: Simula despacho múltiple en lenguajes de despacho simple (Java, C#, TS). Es el núcleo que diferencia Visitor de Strategy o Command.
...
* **Confundir con Strategy o Composite**: Usar Visitor para variar lógica por tipo sin traversal, o para modelar jerarquías sin operaciones externas.
  * Fix: Si varía algoritmo sin estructura → Strategy. Si modela parte-todo → Composite. Si aplica múltiples operaciones a estructura estable → Visitor.
