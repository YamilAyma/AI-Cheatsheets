---
title: "command"
description: "El patrón Command es un patrón de comportamiento que encapsula una petición o acción como un objeto independiente, desacoplando el emisor de la petición del receptor que la ejecuta."
...
Comandos se publican como mensajes, se consumen asíncronamente y se procesan en workers separados. Patrón base de [CQRS](/arquitectura/patrones_distribuidos/cqrs "CQRS es un patrón de arquitectura que segrega las operaciones de lectura (queries) de las de escritura (commands) en modelos independientes, permitiendo optimizar cada lado por separado.") y Event Sourcing.
...
* **[State](/arquitectura/design_patterns/comportamiento/state "El patrón State es un patrón de comportamiento que permite a un objeto alterar su comportamiento cuando su estado interno cambia, haciendo que parezca que el objeto cambia de clase.") Leakage entre Comandos**: Variables estáticas o caché global compartido entre ejecuciones concurrentes.
...
* **Confundir con [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.") o [Memento](/arquitectura/design_patterns/comportamiento/memento "El patrón Memento es un patrón de comportamiento que permite capturar y externalizar el estado interno de un objeto sin violar su encapsulamiento, de modo que el objeto pueda ser restaurado a ese estado en un momento posterior.")**: Usar Command para variar algoritmos o para capturar estado sin intención de ejecución.
  * Fix: Si varía lógica → [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza."). Si solo guarda/restaura estado → [Memento](/arquitectura/design_patterns/comportamiento/memento "El patrón Memento es un patrón de comportamiento que permite capturar y externalizar el estado interno de un objeto sin violar su encapsulamiento, de modo que el objeto pueda ser restaurado a ese estado en un momento posterior."). Si encapsula petición ejecutable/reversible → Command. No mezcle propósitos.
...
* **Pruebe Replay y Fallos, no solo Éxito**: Valide idempotencia, compensación, timeouts, circuit open y recuperación. Detecte order-dependencies y [state](/arquitectura/design_patterns/comportamiento/state "El patrón State es un patrón de comportamiento que permite a un objeto alterar su comportamiento cuando su estado interno cambia, haciendo que parezca que el objeto cambia de clase.")-leaks temprano.
