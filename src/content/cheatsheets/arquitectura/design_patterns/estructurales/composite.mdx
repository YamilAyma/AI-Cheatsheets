---
title: "composite"
description: "El patrón Composite es un patrón estructural que permite componer objetos en estructuras de árbol para representar jerarquías parte-todo."
...
| **[Iterator](/arquitectura/design_patterns/comportamiento/iterator "El patrón Iterator es un patrón de comportamiento que proporciona una forma estandarizada de recorrer secuencialmente los elementos de una estructura agregada sin exponer su representación interna.")/Traversal** | Expone iterador externo (`depthFirst()`, `breadthFirst()`). Separa recorrido de estructura. | Búsquedas complejas, lazy loading, streaming de nodos grandes. | Overhead de estado de iteración. Requiere gestión de cursores o generadores. |
| **Lazy/Deferred** | Hijos se cargan o calculan bajo demanda. `children` es un [proxy](/arquitectura/design_patterns/estructurales/proxy "El patrón Proxy es un patrón estructural que proporciona un sustituto o intermediario para controlar el acceso a un objeto real, añadiendo comportamiento transversal sin modificar su implementación original.") o thunk. | Árboles masivos, sistemas de archivos remotos, UIs virtuales. | Complejidad de invalidación. Fallos de red/carga pueden romper traversal. |
| **Inmutable/Functional** | Cada mutación retorna nuevo árbol. Sin estado compartido. | [Redux](/web/frontend/general/react/redux "Redux es una librería de JavaScript para gestionar el estado de la aplicación.")/[Zustand](/web/frontend/general/zustand "¡Claro que sí! Zustand es una librería de gestión de estados para React (y vainilla JavaScript) conocida por su simplicidad, ligereza y rendimiento.") stores, versionado, replay/debugging seguro. | Presión en GC. Requiere structural sharing (trie/hamt) para eficiencia. |
| **Chunked/Partitioned** | Divide árboles grandes en subárboles procesables en paralelo. | Renderizado de escenas 3D, compilación de ASTs, procesamiento batch. | Complejidad de sincronización. Requiere merge [strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.") para resultados. |
...
* **[Adapter](/arquitectura/design_patterns/estructurales/adapter "El patrón Adapter es un patrón estructural que permite la cooperación entre objetos con interfaces incompatibles, actuando como un traductor o puente que convierte las llamadas de un contrato a otro.")**: Traduce interfaces incompatibles. Enfocado en compatibilidad externa.
* **[Bridge](/arquitectura/design_patterns/estructurales/bridge "El patrón Bridge es un patrón estructural que desacopla una abstracción de su implementación, permitiendo que ambas dimensiones varíen de forma completamente independiente.")**: Desacopla abstracción e implementación. Enfocado en variación independiente de ejes.
...
* **[Visitor](/arquitectura/design_patterns/comportamiento/visitor "El patrón Visitor es un patrón de comportamiento que separa algoritmos de las estructuras de objetos sobre las que operan, permitiendo añadir nuevas operaciones sin modificar las clases de los elementos visitados.")**: Externaliza operaciones sobre estructuras estables. Enfocado en Open/Closed para algoritmos.
* **[Iterator](/arquitectura/design_patterns/comportamiento/iterator "El patrón Iterator es un patrón de comportamiento que proporciona una forma estandarizada de recorrer secuencialmente los elementos de una estructura agregada sin exponer su representación interna.")**: Separa traversal de estructura. Enfocado en acceso secuencial sin exponer implementación interna.
...
* **Confundir con [Decorator](/arquitectura/design_patterns/estructurales/decorator "El patrón Decorator es un patrón estructural que permite añadir responsabilidades a objetos de forma dinámica, sin modificar su clase original ni crear jerarquías de subclases infinitas.") o [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza.")**: Usar Composite para añadir comportamiento dinámico o variar algoritmos.
  * Fix: Si envuelve un solo objeto para extender → [Decorator](/arquitectura/design_patterns/estructurales/decorator "El patrón Decorator es un patrón estructural que permite añadir responsabilidades a objetos de forma dinámica, sin modificar su clase original ni crear jerarquías de subclases infinitas."). Si intercambia lógica → [Strategy](/arquitectura/design_patterns/comportamiento/strategy "El patrón Strategy es un patrón de comportamiento que define una familia de algoritmos intercambiables, los encapsula y los hace sustituibles en tiempo de ejecución sin alterar el código del cliente que los utiliza."). Si anida partes en un todo → Composite.
