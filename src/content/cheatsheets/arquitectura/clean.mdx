---
title: "clean"
description: "La Arquitectura Limpia es una filosofía de diseño de software que propone una organización de código basada en capas concéntricas, donde las dependencias siempre apuntan hacia adentro."
...
1. **Independiente de Frameworks**: La arquitectura no depende de la existencia de alguna librería o framework. No estás atado a Express, Spring, [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario."), [Angular](/web/frontend/framework/angular "Angular es un framework de desarrollo de aplicaciones web de código abierto, desarrollado por Google, para construir aplicaciones de una sola página (SPA) y aplicaciones móviles de alto rendimiento."), etc.
...
4. **Independiente de la Base de Datos**: Las reglas de negocio no saben nada sobre la base de datos que utilizas ([SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros."), NoSQL, ORM, etc.).
...
* **Contenido**:
  ...
  * **Interfaces de Puertos de Entrada (`Input Port`)**: Definidas por los casos de uso para que los "drivers" (controladores de UI/[API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.")) las invoquen. Toman un "Request Model" como entrada.
...
* **Contenido**:
  * **Controladores (Controllers) / APIs / Gateways UI**: Toman la entrada de la UI/[API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") (ej. solicitud HTTP), la convierten en un "Request Model" para el Caso de Uso, e invocan el Caso de Uso.
  ...
  * **Pasarelas de Base de Datos (Database Gateways / Repositories Implementations)**: Implementan las interfaces `IXXXRepository` definidas en la capa de Casos de Uso, traduciendo las operaciones de dominio a llamadas a la base de datos ([SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros."), ORM).
...
* **Cambios**: Cambian cuando la UI, el formato de la base de datos o una [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") externa cambian.
* **Ejemplos**:
  * `OrderController` (toma JSON HTTP, llama a `CrearPedidoInteractor`)
  * `ProductPresenter` (convierte `ProductResponseModel` a `ProductViewModel` para [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.")/[Angular](/web/frontend/framework/angular "Angular es un framework de desarrollo de aplicaciones web de código abierto, desarrollado por Google, para construir aplicaciones de una sola página (SPA) y aplicaciones móviles de alto rendimiento."))
  * `SQLUserRepository` (implementa `IUserRepository`, usando JPA/EF Core para interactuar con [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.")).
...
* **Contenido**:
  ...
  * **Web Frameworks**: Servidores web ([Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente."), Express.js, ASP.NET Core), frameworks de UI ([React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario."), Vue, [Angular](/web/frontend/framework/angular "Angular es un framework de desarrollo de aplicaciones web de código abierto, desarrollado por Google, para construir aplicaciones de una sola página (SPA) y aplicaciones móviles de alto rendimiento.")).
...
* **Ejemplos**:
  * La configuración de `application.properties` en [Spring Boot](/web/backend/framework/spring/spring-boot "¡Absolutamente! Spring Boot es un framework increíble para construir aplicaciones Java robustas y de alto rendimiento de manera rápida y eficiente.").
  * Las tablas y conexiones [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.").
  * Los componentes JSX de [React](/web/frontend/general/react/react "¡Excelente! React es la biblioteca de JavaScript más popular para construir interfaces de usuario.") que usan `useState`.
...
* *Ejemplo de DIP:*
  ...
  4. `CasoDeUso` recibe una `IUserRepository`, sin saber que es una implementación [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.").
...
* **Arquitectura [Hexagonal](/arquitectura/hexagonal "La Arquitectura Hexagonal, o Arquitectura de Puertos y Adaptadores, es un patrón de diseño de software creado por Alistair Cockburn.") (Ports & Adapters)**: Un precursor y concepto hermano de la Arquitectura Limpia. Se enfoca en aislar el núcleo de la aplicación de los detalles externos a través de "puertos" (interfaces) y "adaptadores" (implementaciones).
* **Arquitectura [Cebolla](/arquitectura/cebolla "La Arquitectura Cebolla, propuesta por Jeffrey Palermo, es un patrón de diseño de software que organiza el código en capas concéntricas, similar a las capas de una cebolla.") (Onion Architecture)**: Otro modelo concéntrico muy similar, con énfasis en el dominio como el núcleo.
* **[DDD](/arquitectura/ddd "Domain-Driven Design (DDD) es un enfoque de desarrollo de software que prioriza el dominio del negocio y su lógica sobre la tecnología subyacente.") (Domain-Driven Design)**: A menudo se utiliza junto con la Arquitectura Limpia para modelar dominios complejos, ya que la capa de Entidades/Dominio es el corazón de ambos.
