---
title: "hexagonal"
description: "La Arquitectura Hexagonal, o Arquitectura de Puertos y Adaptadores, es un patrón de diseño de software creado por Alistair Cockburn."
...
* **Flexibilidad Tecnológica**: Permite cambiar tecnologías externas (ej. de [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.") a NoSQL, de REST a gRPC, de CLI a Web UI) con mínimo impacto en la lógica de negocio.
...
* **Contenido**:
  ...
  * **Adaptadores Conducidos / Salida (Driven/Outbound Adapters / Secondary Adapters)**: Implementan los puertos de salida. Son los "dispositivos" o "servicios" que la aplicación usa.
    ...
    * *Ejemplos:* Implementación de un `ProductRepository` con JPA/Entity Framework, cliente HTTP para una [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") externa, un servicio de envío de correos.
...
4. **Adaptador Conducido (ej. Implementación de Repositorio)**:
   ...
   * Convierte el objeto de dominio al formato específico de la infraestructura (ej. JPA Entity, [SQL](/base_datos/sql "SQL es el lenguaje estándar utilizado para comunicarse con y gestionar bases de datos relacionales (RDBMS) como MySQL, PostgreSQL, Oracle, SQL Server, SQLite, entre otros.")).
   * Interactúa con el sistema externo (BD, [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") de terceros).
...
* **Mayor Mantenibilidad**: Los cambios en los detalles de la infraestructura (ej. un nuevo ORM, un cambio en la [API](/conocimiento/api "Una API (Application Programming Interface) es un conjunto de reglas, protocolos y herramientas que permite que diferentes aplicaciones de software se comuniquen entre sí.") de un proveedor externo) no afectan la lógica de negocio.
...
* **Arquitectura Limpia ([Clean](/arquitectura/clean "La Arquitectura Limpia es una filosofía de diseño de software que propone una organización de código basada en capas concéntricas, donde las dependencias siempre apuntan hacia adentro.") Architecture)**: Robert C. Martin se inspiró en la Arquitectura Hexagonal y la Arquitectura [Cebolla](/arquitectura/cebolla "La Arquitectura Cebolla, propuesta por Jeffrey Palermo, es un patrón de diseño de software que organiza el código en capas concéntricas, similar a las capas de una cebolla."), entre otras, para formar la Arquitectura Limpia. Comparten el principio central de independencia de la infraestructura y capas concéntricas con la regla de dependencia.
* **Arquitectura [Cebolla](/arquitectura/cebolla "La Arquitectura Cebolla, propuesta por Jeffrey Palermo, es un patrón de diseño de software que organiza el código en capas concéntricas, similar a las capas de una cebolla.") (Onion Architecture)**: Propuesta por Jeffrey Palermo. También se centra en la inversión de dependencias y un núcleo de dominio. La principal diferencia es su enfoque explícito en la persistencia como una capa separada dentro del "interior" pero fuera del dominio puro.
